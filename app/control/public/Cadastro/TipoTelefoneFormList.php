...
class TipoTelefoneFormList extends TPage
{
    private $form;
    private $datagrid;
    ...
    public function __construct()
    {
        ...
        $this->form = new BootstrapFormBuilder('form_search_TipoTelefone');
        $this->form->setFormTitle(_t('Phone type'));
    ...
    public function onSave($param)
    {
        try
        {
            ...
            {
                TTransaction::open('avaliafit');
                $tipo_telefone = TipoTelefone::where('user_id = '.$user.' and id <> '.(int)$data->id.' and nome','ilike', $data->nome)->first();
                if($tipo_telefone)
                    throw new Exception(_t('There is already a registration with that name, check!'));

                if((int)$data->id > 0)
                    $object = new TipoTelefone($data->id);
                else
                    $object = new TipoTelefone();
    ...
    public function onReload($param = null)
    {
        try
        {
            ...
            $repository = new TRepository('TipoTelefone');
            $limit = 10;
            $user = TSession::getValue('userid');
    ...
    public function onEdit($param)
    {
        ...
        if((int)$key > 0)
        {
            ...
            $object = new TipoTelefone((int)$key);            
