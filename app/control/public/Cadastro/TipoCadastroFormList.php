...
class TipoCadastroFormList extends TPage
{
    private $form;
    private $datagrid;
    ...
    public function __construct()
    {
        ...
        $this->form = new BootstrapFormBuilder('form_search_TipoCadastro');
        $this->form->setFormTitle(_t('Registration type'));
    ...
    public function Delete($param)
    {
        if((int)$param['id'] > 0)
        {
            ...
            $object = new TipoCadastro($param['id']);
            $object->delete();
    ...
    public function onSave($param)
    {
        try
        {
            ...
            {
                TTransaction::open('avaliafit');
                $tipo_cadastro = TipoCadastro::where('user_id = '.$user.' and id <> '.(int)$data->id.' and nome','ilike', $data->nome)->first();
                if($tipo_cadastro)
                    throw new Exception(_t('There is already a registration with that name, check!'));

                if((int)$data->id > 0)
                    $object = new TipoCadastro($data->id);
                else
                    $object = new TipoCadastro();
    ...
    {
        TSession::setValue('TipoCadastroFormList_filter', null);
    ...
    public function onReload($param = null)
    {
        try
        {
            ...
            $repository = new TRepository('TipoCadastro');
            $limit = 10;
            $user = TSession::getValue('userid');
    ...
    public function onEdit($param)
    {
        ...
        if((int)$key > 0)
        {
            ...
            $object = new TipoCadastro((int)$key);            
