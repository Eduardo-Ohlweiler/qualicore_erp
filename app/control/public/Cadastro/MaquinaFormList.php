...
class MaquinaFormList extends TPage
{
    private $form;
    private $datagrid;
    ...
    public function __construct()
    {
        ...
        $this->form->setFormTitle(_t('Machine'));
    ...
    public function onSave($param)
    {
        try
        {
            ...
            {
                TTransaction::open('permission');
                $maquina = Maquina::where('id <> '.(int)$data->id.' and nome','ilike', $data->nome)->first();
                if($maquina)
                    throw new Exception(_t('There is already a registration with that name, check!'));

                if((int)$data->id > 0)
                    $object = new Maquina($data->id);
                else
                    $object = new Maquina();
