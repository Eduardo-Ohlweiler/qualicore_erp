...
class MotivoCancelamentoFormList extends TPage
{
    private $form;
    private $datagrid;
    ...
    public function __construct()
    {
        ...
        $this->form = new BootstrapFormBuilder('form_search_MotivoCancelamento');
        $this->form->setFormTitle(_t('Reason for Cancellation'));
        ...
        $motivo->addValidation(_t('Reason'), new TRequiredValidator);
        $bloqueado->addValidation(_t('Blocked'), new TRequiredValidator);

        $this->form->addFields( [new TLabel(_t('ID'))],             [$id]);
        $this->form->addFields( [new TLabel(_t('Reason')  .' (*)')],  [$motivo] );
        ...
        $col_motivo       = new TDataGridColumn('motivo',       _t('Reason'),     'left',     '95%');
    ...
    public function onSave($param)
    {
        try
        {
            ...
            {
                TTransaction::open('permission');
                $motivo_salvo = MotivoCancelamento::where('id <> '.(int)$data->id.' and motivo','ilike', $data->motivo)->first();
                if($motivo_salvo)
                    throw new Exception(_t('There is already a registration with that name, check!'));

                if((int)$data->id > 0)
                    $object = new MotivoCancelamento($data->id);
                else
                    $object = new MotivoCancelamento();
    ...
    public function onReload($param = null)
    {
        try
        {
            ...
            $repository = new TRepository('MotivoCancelamento');
            $limit = 10;
            $criteria = new TCriteria;

            $criteria->setProperty('limit', $limit);
            $criteria->setProperty('offset', (isset($param['offset'])) ? (int) $param['offset'] : 0);
            $criteria->setProperty('order', 'motivo asc');
    ...
    public function onEdit($param)
    {
        ...
        if((int)$key > 0)
        {
            ...
            $object = new MotivoCancelamento((int)$key);            

            TTransaction::close();
            if($object)
            {
                $data->id        = $object->id;
                $data->motivo      = $object->motivo;
